...
> **定位**: 图形化入口的现行分工。`pnpm admin` 用于隔离组件实验室；`pnpm prototype:carbon`
> 用于在完整页面上下文里审查可见对象的全局组件样式。
...
## `pnpm prototype:carbon`: full-page visual review

`pnpm prototype:carbon` 启动的是完整页面 prototype。它不是让审核人员先认识
...
`tokens/`、`primitives/`、`composites/` 和必要契约，再运行 `pnpm build` 与
`pnpm check:all`。
