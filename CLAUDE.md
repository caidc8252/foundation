...
composites · patterns), where to find each thing fast (`dist/catalog.md`), how to
assemble a page, and how to check your work.

Quick map:
- **What may I use?** → the closed set in [`AGENTS.md`](AGENTS.md); enumerated names in [`dist/catalog.md`](dist/catalog.md) / [`dist/catalog.json`](dist/catalog.json).
...
- **Changing a token/contract** → [`governance/token-change.md`](governance/token-change.md). Edits to values go in `tokens/`, then `pnpm build`; never hand-edit `dist/`.
