...
| `list-filter` | The "quick bar + applied chips" filtering apparatus of a list page. | `.condition-band` `.condition-band__toolbar` `.search-input` `.search-input__icon` `.input` `.select` `.condition-band__spacer` `.applied-filters` `.applied-filters__label` `.filter-chip` | [contract](../composites/list-filter.md) |
