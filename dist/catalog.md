...
| `calendar` | The base month grid — a weekday header over a 6×7 day matrix with month navigation. | `.calendar` `.calendar__nav` `.calendar__nav-btn` `.calendar__caption` `.calendar__weekdays` `.calendar__weekday` `.calendar__week` `.calendar__day` `.calendar__day--today` `--selected` `--range-start` `--range-end` `--range-middle` `--outside` `--disabled` `--focused` `--cell-size` `--cell-radius` | [contract](../primitives/calendar.md) |
...
## L2.5 · Composites (16) — page building blocks
