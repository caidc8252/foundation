...
- **promote** does not touch source itself — it emits a brief so a person or an AI can apply the token/composite overrides into the governed source (`tokens/`, `composites/…`) and commit. This is the "tell an AI what to read" step.
- **finalize** requires a clean git tree and refuses if the draft overrides do not yet match the promoted source.
