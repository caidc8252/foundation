...
### aspect-ratio · primitive · tier 100
A layout box that locks its content to a fixed width:height ratio (16/9 video thumbnails, 1/1 avatars-in-cards, 4/3 media tiles).
class: `.aspect-ratio`
example → `primitives/aspect-ratio.html` (非平凡使用去读、取规范实例)

### carousel · primitive · tier 100
A horizontally (or vertically) scrollable track of equal-width slides, with optional prev/next buttons and dot indicators.
class: `.carousel` `.carousel--vertical` `.carousel__dot` `.carousel__dot--active` `.carousel__dots` `.carousel__item` `.carousel__nav` `.carousel__next` `.carousel__prev` `.carousel__track` `.carousel__viewport`
example → `primitives/carousel.html` (非平凡使用去读、取规范实例)

### resizable · primitive · tier 100
A draggable split-pane layout: sibling panels separated by a hairline handle the user drags to redistribute space.
class: `.resizable-group` `.resizable-group--vertical` `.resizable-handle` `.resizable-handle--horizontal` `.resizable-handle__grip` `.resizable-panel`
example → `primitives/resizable.html` (非平凡使用去读、取规范实例)

### scroll-area · primitive · tier 100
A styled overflow container that swaps the OS scrollbar for a thin, token-skinned one that matches the design system.
class: `.scroll-area` `.scroll-area__scrollbar` `.scroll-area__thumb` `.scroll-area__viewport`
example → `primitives/scroll-area.html` (非平凡使用去读、取规范实例)

### hover-card · primitive · tier 100
A rich popover that opens on hover (and keyboard focus) over a trigger — for preview cards and detail popovers.
class: `.hover-card`
example → `primitives/hover-card.html` (非平凡使用去读、取规范实例)
...
Square, filled entity/object identity mark for companies, apps, device models, etc.
class: `.object-tile` `.object-tile--cat-1` `.object-tile--cat-2` `.object-tile--cat-3` `.object-tile--cat-4` `.object-tile--cat-5` `.object-tile--cat-6` `.object-tile--lg` `.object-tile--md` `.object-tile--neutral` `.object-tile--sm` `.object-tile__icon`
example → `primitives/object-tile.html` (非平凡使用去读、取规范实例)

### date-time-picker · primitive · tier 100
A single date + time field.
class: `.date-picker` `.date-presets` `.date-presets__item` `.date-time-row` `.date-time-row__label` `.date-trigger` `.date-trigger--clearable` `.date-trigger--invalid` `.date-trigger--lg` `.date-trigger--md` `.date-trigger--sm` `.date-trigger__clear` `.date-trigger__icon` `.date-trigger__value` `.date-trigger__value--placeholder`
example → `primitives/date-time-picker.html` (非平凡使用去读、取规范实例)

### time-picker · primitive · tier 100
A time-only field built on the native <input type="time">.
class: (见范例)
example → `primitives/time-picker.html` (非平凡使用去读、取规范实例)
...
### pagination · composite · tier 100
Move through pages of a collection, and set page size.
class: `.pagination` `.pagination__current` `.pagination__ellipsis` `.pagination__page`
example → `composites/pagination.html` (非平凡使用去读、取规范实例)

### diff · composite · tier 100
A compact before → after comparison: the old value struck through, an arrow, the new value highlighted.
class: `.diff` `.diff--inline` `.diff__arrow` `.diff__col` `.diff__col--new` `.diff__col--old` `.diff__label` `.diff__value` `.diff__value--empty`
example → `composites/diff.html` (非平凡使用去读、取规范实例)

### feed-list · composite · tier 100
A vertical list of event rows — each a tone-coded icon, a heading line, a body, and a right-aligned time, with optional row actions.
class: `.feed-item` `.feed-item--read` `.feed-item__actions` `.feed-item__body` `.feed-item__head` `.feed-item__icon` `.feed-item__icon--error` `.feed-item__icon--info` `.feed-item__icon--neutral` `.feed-item__icon--success` `.feed-item__icon--warning` `.feed-item__main` `.feed-item__time` `.feed-item__title` `.feed-item__trailing` `.feed-list`
...
### product-card · composite · tier 100
A catalog tile for one sellable thing: a product image, its name / SKU / short description, a price, and an add-to-cart affordance — laid out as a card and tiled into a responsive grid.
class: `.btn` `.product-card` `.product-card__add` `.product-card__body` `.product-card__cta` `.product-card__description` `.product-card__glyph` `.product-card__image` `.product-card__name` `.product-card__options` `.product-card__placeholder` `.product-card__price` `.product-card__price-row` `.product-card__ribbon` `.product-card__ribbon--info` `.product-card__ribbon--success` `.product-card__ribbon--warning` `.product-card__sku` `.product-grid`
example → `composites/product-card.html` (非平凡使用去读、取规范实例)

### theme-toggle · composite · tier 100
An icon-only button that flips the app between light and dark.
class: `.theme-toggle`
example → `composites/theme-toggle.html` (非平凡使用去读、取规范实例)
