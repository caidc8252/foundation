...
- **Artifact (self-contained HTML)** — use `.hover-card` for the popup surface (positioned by the consumer's own layout/JS), on top of the inlined `dist/tokens.inline.css`. The reference CSS expresses the static skin only (surface, border, radius, shadow, padding, type); open/close and positioning are not reproduced.
