...
- **Artifact (self-contained HTML)** — use `.toggle` + `.toggle--<variant>` + `.toggle--<size>` on a `<button>`, on top of the inlined `dist/tokens.inline.css`. The pressed state is `aria-pressed="true"` (the static stand-in for `data-pressed`). Group looks: wrap items in `.toggle-group` + `.toggle-group--<variant>`; the item classes `.toggle--in-<variant>` express the group-driven skin since a self-contained artifact can't read an ancestor `data-variant` selector dynamically. Same token recipe, same names.
