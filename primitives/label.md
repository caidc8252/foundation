...
- **Artifact (self-contained HTML)** — use `<label class="label">` in `../primitives/primitives.css`, on top of the inlined `dist/tokens.inline.css`. Same `font-sans` / `text-sm` / weight-500 / `select-none` recipe. Disabled dimming is expressed via `.label--disabled` (static stand-in for the React group/peer variants, which need live control state). For the full stacked field, compose with `.field` / `.field__required` / `.field__hint` / `.field__error`.
