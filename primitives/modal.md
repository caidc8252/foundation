...
- **footer** — the action shelf; `surface-3` ground, top `line-subtle` hairline. Stacks reversed on mobile, right-aligned row on `sm`+. Holds `.btn`s (typically a `secondary` cancel + a `primary` or `danger` confirm); for a destructive confirm prefer an `AlertDialog`.
